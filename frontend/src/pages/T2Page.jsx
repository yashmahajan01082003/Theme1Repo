...
import VideoPlayerDashboard from './VideoPlayerDashboard';
import DustyDashboard from '../components/Theme2/T2Dashboard';


const T2Page = () => {
    const [currentVideo, setCurrentVideo] = useState('/assets/T2Intro.mp4');
    ...
    return (
        <div className="w-full min-h-screen overflow-hidden bg-black relative">
            {/* Video layer */}
            <div
                className={`absolute inset-0 transition-opacity duration-700 ease-out ${videoFadingOut ? 'opacity-0' : 'opacity-100'
                    }`}
            >
                <video
                    ref={videoRef}
            ...
            {showDashboard && (
                <div
                    className={`absolute inset-0 z-50 transition-opacity duration-700 ease-out ${dashboardFadingIn ? 'opacity-100' : 'opacity-0'
                        }`}
                >
                    <DustyDashboard onClose={closeDashboard} onPlayVideo={playVideo} />
                </div>
            )}
        </div>
    );
